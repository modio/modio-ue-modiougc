...
custom_edit_url: https://github.com/modio/modio-ue4-internal/blob/develop/Plugins/ModioUGC/Doc/doc_root/en-us/ue-modio-ugc/getting-started.mdx
---

# Getting Started

The **ModioUGC** plugin provides a comprehensive framework for the entire User-Generated Content lifecycle in Unreal Engine. It bridges the gap between content creation in the editor and dynamic loading during gameplay.

To achieve this, the system is divided into two main components:
- **Creation Tools**: An **Editor Wizard** and specialized **Packaging Tools** that transform standard Unreal plugins into platform-optimized UGC packages.
- **Runtime Subsystem**: A robust **UGC Subsystem** and **Provider** architecture that automatically manages discovery, mounting, and registration of content without requiring the engine to restart.

This guide follows the natural workflow, starting with content creation then moving onto accessing your mounted assets at runtime.
...
You can also compress the contents of this folder into a `.zip` archive to upload to [mod.io](https://mod.io).
...
4. Lastly, when testing in editor with UGC that has been cooked with iostore enabled, you must launch your editor instance with the following commandline argument:
```text
-UseIoStore
```

:::important
Matching Io Store settings is critical: the project and the UGC must share the same Io Store configuration to remain compatible. A project cannot mount UGC if one has Io Store enabled and the other does not.
:::
