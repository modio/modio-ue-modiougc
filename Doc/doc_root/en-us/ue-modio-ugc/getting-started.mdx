...
:::important
Matching these settings is critical: the project and the UGC must share the same Io Store configuration to remain compatible. A project cannot mount UGC if one has Io Store enabled and the other does not.
:::
